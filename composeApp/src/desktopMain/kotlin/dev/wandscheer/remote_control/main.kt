...
fun firstLocalIPv4(): Inet4Address =
    NetworkInterface.getNetworkInterfaces().asSequence()          // all NICs
...
fun ipv4WithDefaultGateway(): Inet4Address {
    ...
    val ifaceIp = text
        .lineSequence()
        .dropWhile { !it.trimStart().startsWith("Active Routes:") }
        .firstOrNull { it.trimStart().startsWith("0.0.0.0") }
        ?.trim()
...
fun main() = application {
    ...
    val udp = remember { UdpChannel.open(bindAddress = ipv4WithDefaultGateway(), bindPort = 6669) }
